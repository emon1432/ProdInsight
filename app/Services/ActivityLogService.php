    ...
    public static function record(string $event, $model = null, array $extra = [])
    {
        try {
            $data = array_merge([
                ...
                'event'       => $event,
                'title' => match ($event) {
                    'deleted'        => 'Deleted ' . class_basename($model::class),
                    'permanently_deleted'  => 'Permanently Deleted ' . class_basename($model::class),
                    'restored'       => 'Restored ' . class_basename($model::class),
                    'created'       => 'Created ' . class_basename($model::class),
                    'updated'       => 'Updated ' . class_basename($model::class),
                    default          => ucfirst($event) . ' ' . class_basename($model::class),
                },
